...
bool launch_convrot_activation_quant_typed(
    X_T const* x, int8_t* x_q, float* x_scale, void const* H,
    ...
    cudaStream_t stream)
{
    if (input_dtype == 1) {
        return launch_convrot_activation_quant_typed(
            static_cast<__half const*>(x), x_q, x_scale, H, M, K, group_size, stream);
    }
    if (input_dtype == 2) {
        return launch_convrot_activation_quant_typed(
            static_cast<uint16_t const*>(x), x_q, x_scale, H, M, K, group_size, stream);
    }
    return launch_convrot_activation_quant_typed(
        static_cast<float const*>(x), x_q, x_scale, H, M, K, group_size, stream);
}
