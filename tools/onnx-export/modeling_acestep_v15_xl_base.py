...
class AceStepDiTModel(AceStepPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # ──────────────────────────────────────────────────────────────
        # Downsample self-attn padding mask to post-patch resolution.
        #
        # The runtime attention_mask is [B, T] (latent resolution, pre-patch).
        # The model attends at seq_len = T / patch_size (post-patch). We need
        # a [B, seq_len] mask to feed into create_4d_mask's
        # `attention_mask.view(B, 1, 1, seq_len)` call.
        #
        # IMPORTANT: do NOT reassign the `attention_mask` name here. dynamo
        # ties input_latents[1] and attention_mask[1] to the SAME symbolic
        # Dim (see export_dit.py dynamic_shapes), so mutating the shape would
        # break the input-contract and force dynamo to specialize on the
        # trace value (causing "tensor has 512 but profile has min=64,
        # opt=2048, max=3000" at TRT build time). Use a new local name.
        #
        # Implementation notes (after FIVE iterations):
        #  - F.max_pool1d: dynamo symbolic-shape rule unreliable, causes
        #    specialization.
        #  - F.adaptive_max_pool1d: dynamo decomposes to
        #    aten::adaptive_max_pool2d which has no ONNX translation.
        #  - tensor.unfold(P, P) + max: WORKS for dynamo+ONNX export, BUT
        #    internally generates a Range op with a 1-dim `end` argument
        #    (T - P as a [1]-shape tensor). TRT 11's Range op requires all
        #    three args (start/end/step) to be scalar 0-dim tensors, and
        #    fails with "Assertion x.nbDims == y.nbDims failed" when `end`
        #    is 1-dim.
        #  - tensor.reshape(B, post, P) + max: requires dynamo to prove
        #    T % P == 0. This works when export_dit.py declares
        #    `seq_len = patch_size * _seq_len_post` (a derived dim), which
        #    tells dynamo upfront that T is always a multiple of P. The
        #    resulting ONNX uses only Concat/Div/ReduceMax/Reshape/Shape/
        #    Squeeze — no Range op, no TRT assertion failure. THIS IS THE
        #    APPROACH USED BELOW.
        #  - reshape+max gives "patch is valid if ANY frame in it is valid"
        #    semantics, matching the C++ host's real_S[b] convention.
        # ──────────────────────────────────────────────────────────────
        attn_mask_post = None
        if attention_mask is not None and self.patch_size > 1:
            # attention_mask: [B, T_pre]
            # post = T_pre // patch_size  (dynamo infers from derived dim)
            # reshape(B, post, P) -> [B, post, P]  (non-overlapping patches)
            # max(dim=-1)         -> [B, post]      (patch valid if any frame valid)
            B_ = attention_mask.shape[0]
            post = attention_mask.shape[1] // self.patch_size
            attn_mask_post = attention_mask.reshape(B_, post, self.patch_size).max(
                dim=-1
            ).values.to(torch.long)
        elif attention_mask is not None:
            # patch_size == 1 (no patching) — mask shape already matches seq_len
            attn_mask_post = attention_mask

        # Initialize mask variables. Do NOT shadow the function parameters
        # (attention_mask / encoder_attention_mask) — the previous code
        # unconditionally set both to None here, which silently disabled all
        # padding masks and caused the TRT engine to ship with dead mask
        # inputs (see dit-trt.h:976-991 "timbre-dilution bug" comment).
        full_attn_mask = None
        sliding_attn_mask = None
        ...
        if is_flash_attn:
            ...
            # 场景 A: Flash Attention 模式
            # -------------------------------------------------------
            # FA 不需要 4D Mask。直接传 2D padding mask [B, L] 即可。
            # 滑动窗口逻辑由 Layer 内部传给 FA kernel 的 sliding_window 参数控制。
            # FA2 expects the post-patch resolution mask (seq_len = T/patch_size).
            full_attn_mask = attn_mask_post
            sliding_attn_mask = attn_mask_post if self.config.use_sliding_window else None
            # Cross-attn: FA2 takes the 2D encoder mask directly at [B, enc_seq_len].
            encoder_attn_bias = encoder_attention_mask
            ...
            # 1. Full Self-Attention (Bidirectional, Global)
            #    attn_mask_post is [B, seq_len] (post-patch), matching seq_len.
            full_attn_mask = create_4d_mask(
                seq_len=seq_len,
                dtype=dtype,
                device=device,
                attention_mask=attn_mask_post,     # [B, seq_len] (post-patch)
                sliding_window=None,
                is_sliding_window=False,
                is_causal=False                    # <--- 关键：双向注意力
            )

            # 2. Sliding Self-Attention (Bidirectional, Local)
            if self.config.use_sliding_window:
                sliding_attn_mask = create_4d_mask(
                    seq_len=seq_len,
                    dtype=dtype,
            ...
            # 3. Cross-Attention mask: [B, 1, seq_len, encoder_seq_len]
            #    Q from hidden_states (seq_len = T/patch_size),
            #    K from encoder_hidden_states (encoder_seq_len = S).
            #    Mask K positions where encoder_attention_mask == 0.
            #
            #    Build directly at the correct (non-square) shape — no
            #    max_len / crop needed. The previous code used the wrong
            #    variable (attention_mask instead of encoder_attention_mask)
            #    and built a square [max_len, max_len] mask then cropped,
            #    which both crashed on shape volume and applied the wrong
            #    padding semantics.
            if encoder_attention_mask is not None:
                B = hidden_states.shape[0]
                enc_pad = encoder_attention_mask.view(B, 1, 1, encoder_seq_len).to(torch.bool)
                enc_valid = enc_pad.expand(B, 1, seq_len, encoder_seq_len)
                min_dtype = torch.finfo(dtype).min
                encoder_attn_bias = torch.full(
                    (B, 1, seq_len, encoder_seq_len), min_dtype, dtype=dtype, device=device
                )
                encoder_attn_bias = encoder_attn_bias.masked_fill(enc_valid, 0.0)
